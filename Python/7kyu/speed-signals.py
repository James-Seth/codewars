...
def speed_limit(speed, signals):
    ...
    for i in signals:
        if (speed - i) >= 10:
            if (speed - i) <= 19: fine += 100
            elif (speed - i) <= 29: fine += 250
            elif (speed - i) >= 30: fine += 500
    return fine

#Other solutions:
def speed_limit(speed, signals):
    return sum(500 if (d:=speed-s) > 29 else 250 if d > 19 else 100 if d > 9 else 0 for s in signals)